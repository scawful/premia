...
void Client::OpenBrowser() {
  auto host = "http%3A%2F%2Flocalhost:8080";
...
std::string Client::get_price_history(const std::string &symbol,
  ...
  string_replace(url, "{ext}", ext ? "true" : "false");
  return send_request(url);
}
...
std::string Client::get_option_chain(
  ...
  string_replace(url, "{includeQuotes}", includeQuotes ? "TRUE" : "FALSE");
  return send_request(url);
}
