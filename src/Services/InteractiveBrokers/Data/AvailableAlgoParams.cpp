...
#include "AvailableAlgoParams.hpp"
...
//! [arrivalpx_params]
void AvailableAlgoParams::FillArrivalPriceParams(
    Order& baseOrder, double maxPctVol, std::string riskAversion,
    std::string startTime, std::string endTime, bool forceCompletion,
    bool allowPastTime, double monetaryValue) {
  ...
  TagValueSPtr tag5(
      new TagValue("forceCompletion", forceCompletion ? "1" : "0"));
  TagValueSPtr tag6(
      new TagValue("allowPastEndTime", allowPastTime ? "1" : "0"));
  TagValueSPtr tag7(
      new TagValue("monetaryValue", std::to_string(monetaryValue)));
...
//! [darkice_params]
void AvailableAlgoParams::FillDarkIceParams(Order& baseOrder, int displaySize,
...
//! [pctvol_params]
void AvailableAlgoParams::FillPctVolParams(Order& baseOrder, double pctVol,
                                           std::string startTime,
...
//! [twap_params]
void AvailableAlgoParams::FillTwapParams(
    Order& baseOrder, std::string strategyType, std::string startTime,
...
//! [vwap_params]
void AvailableAlgoParams::FillVwapParams(Order& baseOrder, double maxPctVol,
                                         std::string startTime,
                                         std::string endTime,
                                         bool allowPastEndTime, bool noTakeLiq,
...
//! [ad_params]
void AvailableAlgoParams::FillAccumulateDistributeParams(
    Order& baseOrder, int componentSize, int timeBetweenOrders,
    bool randomizeTime20, bool randomizeSize55, int giveUp, bool catchUp,
    bool waitForFill, std::string startTime, std::string endTime) {
  baseOrder.algoStrategy = "AD";
  baseOrder.algoParams.reset(new TagValueList());
  TagValueSPtr tag1(
      new TagValue("componentSize", std::to_string(componentSize)));
  TagValueSPtr tag2(
      new TagValue("timeBetweenOrders", std::to_string(timeBetweenOrders)));
  TagValueSPtr tag3(
      new TagValue("randomizeTime20", randomizeTime20 ? "1" : "0"));
  TagValueSPtr tag4(
      new TagValue("randomizeSize55", randomizeSize55 ? "1" : "0"));
...
//! [closepx_params]
void AvailableAlgoParams::FillClosePriceParams(
    Order& baseOrder, double maxPctVol, std::string riskAversion,
...
//! [pctvolpx_params]
void AvailableAlgoParams::FillPriceVariantPctVolParams(
    Order baseOrder, double pctVol, double deltaPctVol, double minPctVol4Px,
    double maxPctVol4Px, std::string startTime, std::string endTime,
...
//! [pctvolsz_params]
void AvailableAlgoParams::FillSizeVariantPctVolParams(
    Order baseOrder, double startPctVol, double endPctVol,
...
//! [pctvoltm_params]
void AvailableAlgoParams::FillTimeVariantPctVolParams(
    Order baseOrder, double startPctVol, double endPctVol,
    std::string startTime, std::string endTime, bool noTakeLiq,
    double monetaryValue) {
...
void AvailableAlgoParams::FillJefferiesVWAPParams(
    Order baseOrder, std::string startTime, std::string endTime,
    double relativeLimit, double maxVolumeRate, std::string excludeAuctions,
    double triggerPrice, double wowPrice, int minFillSize, double wowOrderPct,
  ...
  TagValueSPtr tag3(
      new TagValue("RelativeLimit", std::to_string(relativeLimit)));
  TagValueSPtr tag4(
      new TagValue("MaxVolumeRate", std::to_string(maxVolumeRate)));
...
//! [csfb_inline_params]
void AvailableAlgoParams::FillCSFBInlineParams(
    Order baseOrder, std::string startTime, std::string endTime,
    std::string execStyle, int minPercent, int maxPercent, int displaySize,
    std::string auction, bool blockFinder, double blockPrice, int minBlockSize,
    int maxBlockSize, double iWouldPrice) {
  ...
  TagValueSPtr tag10(
      new TagValue("MinBlockSize", std::to_string(minBlockSize)));
  TagValueSPtr tag11(
      new TagValue("MaxBlockSize", std::to_string(maxBlockSize)));
