        ...
        while (clipper.Step()) {
            for (int row = clipper.DisplayStart; row < clipper.DisplayEnd; row++) {
                ...
                for (int column = 0; column < 6; column++) {
                    ...
                    switch(column) {
                        case 0:
                            draw_symbol_string(symbol); break;
                        case 1:
                            draw_balance_string(account_data.get_position_balances(symbol, "currentDayProfitLoss")); break;
                        case 2:
                            draw_balance_string(account_data.get_position_balances(symbol, "currentDayProfitLossPercentage")); break;
                        case 3:
                            draw_balance_string(account_data.get_position_balances(symbol, "averagePrice")); break;
                        case 4:
                            draw_balance_string(account_data.get_position_balances(symbol, "marketValue")); break;
                        case 5:
                            draw_balance_string(account_data.get_position_balances(symbol, "longQuantity")); break;
                        default:
                            break;
                    }
                }
...
void AccountView::drawAccountPane()
{
    ...
    ImGui::Text("Long Mkt Val: %s", account_data.get_balance_variable("longMarketValue").c_str());
    ImGui::Text("Short Mtk Val: %s", account_data.get_balance_variable("shortMarketValue").c_str());
    ImGui::Text("Cumulative BP: %s", account_data.get_balance_variable("buyingPower").c_str());
    ImGui::Text("Equity: %s, %s%", account_data.get_balance_variable("equity").c_str(),
                                   account_data.get_balance_variable("equityPercentage").c_str());
    ImGui::Text("Margin Balance: %s", account_data.get_balance_variable("marginBalance").c_str());
    ImGui::Separator();
